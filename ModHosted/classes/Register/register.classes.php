...
class Register extends Dbh
{
    ...
    protected function register($name, $email, $password, $user_type)
    {
        ...
        $stmt = $this->connect()->prepare('INSERT INTO USERS(`FULL_NAME`,`USER_ALIAS`,  `CREDENTIAL`, `EMAIL`, `PHONE_NUMBER`, `BIRTHDAY`, `PROFILE_PICTURE`, `BANNER_PICTURE`, `USER_TYPE`, `CREATED_BY`, `LAST_UPDATED_BY`) 
        VALUES (?, ?, ?, ?, NULL, NULL, NULL, NULL, ?, 0, 0);');

        //Codificamos la contraseña para la seguridad de su almacenamiento
        $hashPwd = password_hash($password, PASSWORD_DEFAULT);

        if (!$stmt->execute(array($name, $name, $hashPwd, $email, $user_type))) {
