...
BEGIN
 ...
 /*#######################################
    ...
    IF Oper = 'SO'
	THEN
		SELECT `ID_USER`, `FULL_NAME`, plceholderName(`USER_ALIAS`) as PLACEHOLDER, `USER_ALIAS`,  `CREDENTIAL`, `EMAIL`, `PHONE_NUMBER`, `BIRTHDAY`, `PROFILE_PICTURE`, `BANNER_PICTURE`, `USER_TYPE`, `CREATED_BY`, `LAST_UPDATED_BY` FROM USERS
        WHERE `EMAIL` = emailT;
        LEAVE `sp_User`;   
    END IF;
