      ...
      <script type="text/javascript">
            $(document).ready(function() {
                ...
                $('#Aprobar').click(function(e){
                    ...
                    $('#Denegar').remove();
                    $('#Aprobar').remove();
                    $('#ProcesRevisionForm').append(
                        '<input type="hidden" id="feedback" name="feedback" value="Todo Bien">'+
                        'Fecha y Hora de Publicacion:'+
                        '<input type="datetime-local" id="PubDate" name="PubDate"/>'+
                    '<button type="submit" class="Aprobar" id="submit">Publicar Noticia</button>');
                    $('#PubDate').val(strDate);
                });
                $('#Denegar').click(function(e){
                    e.preventDefault();
                    //Set the Nota status to RA and do the review.
                    $('#Revision').val(0);
                    $('#Denegar').remove();
                    $('#Aprobar').remove();
                    $('#ProcesRevisionForm').append(
                        '<textarea id="feedback" name="feedback" rows="6" cols="50">Comentarios de retroalimentacion</textarea>'
                    +'<button type="submit" class="Denegar" id="submit">Mandar Feedback y Cambiar Estatus</button>');
                });
                $('#ProcesRevisionForm').on('submit',function(e){
                    let timerInterval
                        Swal.fire({
                        title: 'Cargando...',
                        timer: 5000,
...
<body>
    <div class="Contenedor-base">
        ...
        <div class="mainContenedor">
            ...
            <button type="button" style="background-color:gray; color:white;" value="Volver al Portal del Editor" onclick="window.history.go(-1); return false;"><i class="fa fa-arrow-left" aria-hidden="true"></i>Volver al Portal del Editor</button>
            <?php
                if($hasfeedbacks){
                    $me = null;
                    $lastChildren = null;
                    ...
                    for($i=0;$i<count($feedbacks);$i++){
                        echo'<div>';
                        if($feedbacks[$i]['FEEDBACK_BY'] == $me) echo'<h8>Mi Comentario:</h8> <br>';
                        else echo'<h8>Respuesta Reportero:</h8> <br>';
